...
class TokenMixerCNN(nn.Module):
    def __init__(self, dim):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(dim, dim, kernel_size=1, stride=1, padding=0, groups=dim), nn.GELU(), nn.Conv2d(dim, dim, kernel_size=1, stride=1, padding=0))
        self.conv3 = nn.Sequential(
            nn.Conv2d(dim, dim, kernel_size=3, stride=1, padding=1, groups=dim), nn.GELU(), nn.Conv2d(dim, dim, kernel_size=1, stride=1, padding=0))
        self.conv5 = nn.Sequential(
            nn.Conv2d(dim, dim, kernel_size=5, stride=1, padding=2, groups=dim), nn.GELU(), nn.Conv2d(dim, dim, kernel_size=1, stride=1, padding=0))
        self.conv7 = nn.Sequential(
            nn.Conv2d(dim, dim, kernel_size=7, stride=1, padding=3, groups=dim), nn.GELU(), nn.Conv2d(dim, dim, kernel_size=1, stride=1, padding=0))
        self.act = nn.GELU()
    
    def forward(self, x):
        return self.act(self.conv1(x) + self.conv3(x) + self.conv5(x) + self.conv7(x))
